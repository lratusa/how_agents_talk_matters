% main_en.tex -- SCI journal submission version.
% Generated from the frozen paper/main.tex: body (sections, math, numbers,
% tables, bibliography) copied verbatim; only front matter (title page,
% highlights, <=250-word abstract, keywords) and the Declarations block are new.
% Compile: pdflatex main_en.tex (requires a LaTeX distribution)
...
\author{Rui Li, Chenglong Sun, Xueyan Zheng, Dan Liu\\[0.3em]Gi (Beijing) Tech Design Ltd., Beijing, China}
\date{September 2026}

\begin{document}
\maketitle
\pagestyle{myheadings}
\markright{Li et al.: Who Should Review Whom?}

% Running title: Who Should Review Whom? (45 characters incl. spaces: "Who Should Review Whom" = 22)

\section*{Highlights}
\begin{itemize}
\item Farthest-pair review does not beat random pairing under controlled matching.
\item Semantic distance raises error detection, not error correction.
\item Review generates novel correct answers that are rarely delivered.
\item No-review synthesis beats peer review on the harder benchmark.
\item Beneficial Transition Ratio quantifies review's net-zero effect.
\end{itemize}

\begin{abstract}
Instances of the same language model answering the same prompt do not err independently, so mutual review may mostly recycle correlated errors. We ask whether the \emph{topology} of peer review matters: Farthest-Pair Reciprocal Review (FPRR) embeds $N$ independently sampled responses, pairs semantically distant responses for mutual review, and synthesizes a final answer. On MMLU-Pro (500 questions $\times$ 3 seeds) and SuperGPQA (500 $\times$ 4 seeds) under identical cached initial responses, FPRR does not improve aggregate accuracy: it differs from random pairing by $-0.5$ and $-0.7$ percentage points, every paired bootstrap confidence interval crosses zero, and no comparison survives Holm correction; on SuperGPQA no-review synthesis significantly \emph{beats} peer review ($+1.5$pp, $p=0.010$). The mechanism is visible but self-canceling: detection of genuinely wrong peer answers rises with pair distance ($0.00$--$0.11$ in the lowest decile to $0.30$--$0.44$ in the highest), yet whether exposure propagates to repair and adoption is regime-dependent (detection only on MMLU-Pro; all three stages on SuperGPQA), and a reverse chain --- false opposition and destabilization --- scales with the same distance, yielding Beneficial Transition Ratios of $0.54$ and $0.383$. Review creates genuinely new knowledge --- novel correct answers on $0$--$4\%$ of all-wrong questions --- but end-to-end delivery is $\le 1.5\%$. Our pre-registered outcome is Result category D (a null result): the binding constraint is epistemic credit assignment, not routing. Limitations: one model, one decoding configuration, a low-diversity regime, and residual contamination.
\end{abstract}

\noindent\textbf{Keywords:} multi-agent systems; large language models; test-time compute; communication topology; peer review; negative result
...
All authors contributed to study conception and design, experiment execution, data analysis, and manuscript preparation, and approved the final version.

\subsection*{Funding}
This work was conducted and funded by Gi (Beijing) Tech Design Ltd.; no external funding was received.

\subsection*{Competing interests}
All authors are employees of Gi (Beijing) Tech Design Ltd.
